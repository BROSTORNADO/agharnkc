...
const Home = () => {
  ...
  const locations = [
    ...
    'Arafat - عرفات', 'El Mina - الميناء', 'Riyadh - الرياض', 'Elvelouja - الفلوجة',
    'Melah - ملح', 'Tarhil - الترحيل', 'Bouhdida - بوحديدة', 'Tensweylm - تنسويلم',
    'Carefour - كرفور', 'El Matar - المطار', 'Socogim - سوكوجيم', 'Soukouk - الصكوك',
    'Premier - ابريمير', 'Ain Talh - عين الطلح', 'Zaatar - الزعطر', 'Cinquième - سينكيم',
    'Sixième - سيزيم'
  ];
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await axios.get('/api/posts', { params: { location: selectedLocation } });
        setPosts(res.data);
      } catch (error) {
          ...
          <div key={post._id} className="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="text-gray-700 font-bold mb-2 flex items-center">
                <FaMapMarkerAlt className="text-red-500 mr-2" /> {post.location}
              </p>
              <a
