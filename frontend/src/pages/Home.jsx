...
const Home = () => {
  ...
  const locations = [
    ...
    'Riyadh - الرياض', 'Elvelouja - الفلوجة', 'Melah - ملح', 'Tarhil - الترحيل',
    'Bouhdida - بوحديدة', 'Tensweylm - تنسويلم', 'Carefour - كرفور', 'El Matar - المطار',
    'Socogim - سوكوجيم', 'Soukouk - الصكوك', 'Premier - ابريمير', 'Ain Talh - عين الطلح',
    'Zaatar - الزعطر', 'Cinquième - سينكيم', 'Sixième - سيزيم',
  ];
  ...
  // Filter posts by selected location (client-side)
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      ...
      {filteredPosts.length === 0 ? (
        <div className="text-center text-gray-500 mt-8">Aucune publication trouvée</div>
      ) : (
        <>
            ...
            <div className="flex justify-center mt-8">
              ...
              {pageNumbers().map((page) => (
                <button
                  key={page}
                  className={`px-4 py-2 mx-1 rounded ${
                    currentPage === page ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-800'
                  }`}
              ...
              <button
                className={`px-4 py-2 mx-1 rounded ${
                  currentPage === totalPages ? 'bg-gray-300' : 'bg-emerald-400 text-white'
                }`}
                onClick={() => handlePageChange(currentPage + 1)}
      ...
      {fullscreenImages && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50">
          <div {...handleSwipe} className="relative w-full h-full flex items-center justify-center">
            <button className="absolute top-4 right-4 text-white text-2xl" onClick={closeFullscreen}>
              <FaTimes />
            </button>
            <img
